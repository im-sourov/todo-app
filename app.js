...
const themeSwitchIconContent = document.querySelector(`.themeSwitch__icon`).firstElementChild;
...
const invokeIfDark = () => {
    ...
    setElementAttribute(themeSwitchIconContent, `href`, `images/spriteSvg.svg#icon-moon`);
...
const invokeIfLight = () => {
    ...
    toggleClassName(themeSwitchIcon, `themeSwitch__icon--dark`);
    toggleClassName(themeSwitchCaption, `themeSwitch__caption--dark`);
    setElementAttribute(themeSwitchIconContent, `href`, `images/spriteSvg.svg#icon-sun`);
...
const noTask = () => {
    allTask.length <= 0 ? emptyTaskContainer.classList.add(`taskContainer__noTaskActive`) 
...
const createTaskItem = (itemNode) => {
    ...
    const taskItemElementsWithAttributes = 
    taskItemElements.map(value => {
        switch(value.tagName) {
            ...
            case 'DIV':
                ...
                value.addEventListener(`click`, () => {
                    ...
                    allTask.length <= 0 ? localStorage.removeItem(`task`) : localStorage.setItem(`task`, allTask);
                    noTask();
